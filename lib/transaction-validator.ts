...
export class TransactionValidator {
    ...
    /**
     * [CENTRAL CONTROL] Validate System-Level Operations
     * Handles sensitive operations like Backups, Resets, and Global Config.
     * 
     * [Backup Protocol 2026-02-11]
     * - Multi-channel Auth: Header, Query Param, Custom Header
     * - Alphanumeric Key Standard
     */
    static validateSystemOperation(operation: 'BACKUP' | 'RESET', context: { 
        receivedSecret: string, 
        expectedSecret?: string,
        isProduction: boolean 
    }): { success: boolean, error?: string, diagnostics?: any } {
        const { receivedSecret, expectedSecret, isProduction } = context;

        // In Production, expectedSecret MUST be present
        if (isProduction && !expectedSecret) {
            return { success: false, error: '시스템 보안 키가 설정되지 않았습니다. (Expected Secret Missing)' };
        }

        // Authentication logic (Trimmed for tolerance)
        const cleanReceived = receivedSecret.trim();
        ...
        if (isProduction) {
            if (cleanReceived !== cleanExpected) {
                return { 
                    ...
                    error: 'Unauthorized',
                    diagnostics: {
                        receivedLen: cleanReceived.length,

...
// console.log(`☁️ [DB] Delta Sync Success (${syncTasks.length} segments)`);
  ...
  public async init() {
      ...
      // [CRITICAL FIX] Skip initialization if Supabase client is null (during build or misconfigured)
      if (!supabase) {
          console.warn('⚠️ [DB] Skipping DB init: Supabase client is null (Build Phase or Missing Keys)');
          return;
      }

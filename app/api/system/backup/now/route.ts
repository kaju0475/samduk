...
export async function GET(request: NextRequest) {
    ...
    // [DEEP RESET AUTH] Multi-channel verification (Header, Custom Header, Query Param)
    const rawAuthHeader = request.headers.get('authorization');
    const authHeader = rawAuthHeader?.replace('Bearer ', '');
    const xCronAuth = request.headers.get('x-cron-auth');
    const querySecret = request.nextUrl.searchParams.get('secret');

    // 1. All inputs are trimmed to prevent invisible character mismatches (Zombie spaces)
    const received = (authHeader || xCronAuth || querySecret || '').trim();
    const expected = (process.env.CRON_SECRET || '').trim();

    // [CENTRAL CONTROL INTEGRATION] Delegate auth to TransactionValidator
    const validation = TransactionValidator.validateSystemOperation('BACKUP', {
    ...
    if (!validation.success) {
        console.error('[Backup Reset] AUTH REJECTED:', validation.diagnostics);
        return NextResponse.json({ success: false, message: validation.error }, { status: 401 });
    }
    
    console.log('[Backup Reset] AUTH GRANTED via TransactionValidator.');
        ...
        const stream = new ReadableStream({
            async start(controller) {
                try {
                    ...
                    const streamSection = async (key: string, table: string, mapper?: (item: Record<string, unknown>) => unknown) => {
                        try {
